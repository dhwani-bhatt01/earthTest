...
function Home2() {
  const parentStrategyValContractExplorer = "https://goerli.basescan.org/address/0x25adf247ac836d35be924f4b701a0787a30d46a9";
  const parentStrategyValContract = "0x25ADF247aC836D35be924f4b701A0787A30d46a9";
  const staticValutList = ["0x74C5E75798b33D38abeE64f7EC63698B7e0a10f1", "0xe8d223328543Cc10Edaa3292CE12C320CE43A099"];
  ...
  const [approvalWaitingVisible, setapprovalWaitingVisible] = useState<boolean>(false);
  const [depositWithDrawVisible, setDepositWithDrawVisible] = useState<boolean>(false);
  ...
  const [depositWaitingVisible, setdepositWaitingVisible] = useState<boolean>(false);
  const [depositSuccessfullVisible, setdepositSuccessfullVisible] = useState<boolean>(false);
  const [vaultAddress, setVaultAddress] = useState("");
  const [withdrawAmout, setwithdrawAmout] = useState("0");
  const [approvalSuccessfullVisible, setapprovalSuccessfullVisible] = useState<boolean>(false);
  ...
  const getHistoricalData = async () => {
    ...
    fetch(`https://api.covalenthq.com/v1/mantle-testnet/address/${vaultAddress}/transactions_v3/?quote-currency=USD`, { method: 'GET', headers: headers })
      .then((resp) => resp.json())
      .then((data) => {
        console.log(data)
        ...
        const output = data.data.items.slice(0, 10).map((e: any) => {
          ...
          if (e.log_events) {
            ...
            if (e.log_events[e.log_events.length - 1].decoded.name === "Withdraw" || e.log_events[e.log_events.length - 1].decoded.name === "Approval") {
              const prsVal = e.log_events[e.log_events.length - 1].decoded.params;
              value = ethers.utils.formatEther(prsVal[prsVal.length - 1].value);
            } else if (e.log_events[e.log_events.length - 1].decoded.name === "Deposit") {
  ...
  const getValutDetailsByChain2 = async (valultList: string[], provider: any) => {

    const valutListVal = await Promise.all(valultList?.map(async (vaultAddressVal: any) => {
      let userHolding = "";
      let walletBalance = "";
      ...
      const assetAbiResponse = await fetch(data[vaultAddressVal as string].assetAbi);
      ...
      const vaultContract = getContract(vaultAddressVal as string, earthAutoCompoundingVaultPublicJson.abi, localProvider);
      ...
      if (address) {
        ...
        const userShareVal = (new Decimal(totalAssets).mul(new Decimal(share))).div(new Decimal(totalSupply));
        setUserShare(userShareVal.toFixed(18));
        console.log("your holdning", userShareVal.toFixed(18));
        userHolding = userShareVal.toFixed(2);


        const asstsContract = getContract(assetAdress, assetAbiResponseJson.abi, provider);
  ...
  async function getDeployedValut() {
    ...
    const parentStrategy = getContract(parentStrategyValContract as string, earthLpStaking.abi, mantleLocalProvied);
    ...
    const mantleValutList = await getValutDetailsByChain2(staticValutList, mantleLocalProvied);
    setvalutList(mantleValutList);

    //get vault deatils for vault with lp coin
    const mantleDetailValut = await getValutDetailsByChain2(staticLpValut, mantleLocalProvied);
  ...
  const showWarn = (header: string, message: string) => {
    toast.current?.show({ severity: 'warn', summary: header, detail: message, life: 3000 });
  }
  ...
  const checkAllowance = async (valutAddressVal: any) => {
    ...
    let vaultContract = getContract(valutAddressVal as string, earthAutoCompoundingVaultPublicJson.abi, localProvider);
    const assetAdress = await vaultContract.asset();
  ...
  const deposit = async () => {
    ...
    const contract = getContract(vaultAddress as string, earthAutoCompoundingVaultPublicJson.abi, signer);
  ...
  const CustomTabHeader: React.FC<CustomTabHeaderProps> = ({ icon, title }) => {
    return (
      <>
        {icon && <img src={icon} alt="Icon" style={{ marginRight: '10px', width: '28px' }} />}
        <span>{title}</span>
      </>
    );
  };
  ...
  const CustomAccordianHeader1: React.FC<CustomTabHeaderProps> = ({ icon, title }) => {
    return (
      <>
        <div className='dsp mb-4'>
          <div> {icon && <img src={icon} alt="Icon" style={{ marginRight: '10px', width: '28px' }} />}  <span className='primary_header_color'>{title}</span> </div>
          <div><img src={rocketAccordianImg} alt="Icon" style={{ marginRight: '10px', width: '28px' }} />  <span className='mr_10'>13 August 2023 | 14:35:00 UTC</span><img src={externalLinkGreenImg} alt='externalLink' /></div>
          <div><img src={flagGreenImg} alt="Icon" style={{ marginRight: '10px', width: '28px' }} />  <span className='mr_10'>13 August 2023 | 14:37:00 UTC</span> <img src={externalLinkGreenImg} alt='externalLink' /></div>
        </div>
        <div className='dsp'>
          <div> <div className='fnt_wght_500 mb-2'>Starting Balance</div> <div>12.32657812 ETH-CLOUD LP</div></div>
          <div> <div className='fnt_wght_500 mb-2'>Ending Balance </div>  <div>12.33567612 ETH-CLOUD LP</div> </div>
          <div> <div className='fnt_wght_500 mb-2'>Epoch Growth</div>  <div className='primary_header_color'>0.00%</div></div>
          <div> <div className='fnt_wght_500 mb-2'>Epoch APR</div>  <div className='primary_header_color'>0.86%</div></div>
          <div> <div className='fnt_wght_500 mb-2'>Leverage</div>  <div className='primary_header_color'>3.2x</div></div>
        </div>
      </>
    );
  };

  const CustomAccordianHeader2: React.FC<CustomTabHeaderProps> = ({ icon, title }) => {
    return (
      <>
        <div className='dsp mb-4'>
          <div> {icon && <img src={icon} alt="Icon" style={{ marginRight: '10px', width: '28px' }} />}  <span className='primary_header_color'>{title}</span> </div>
          <div><img src={rocketAccordianImg} alt="Icon" style={{ marginRight: '10px', width: '28px' }} />  <span className='mr_10'>12 August 2023 | 22:30:00 UTC</span><img src={externalLinkGreenImg} alt='externalLink' /></div>
          <div><img src={flagGreenImg} alt="Icon" style={{ marginRight: '10px', width: '28px' }} />  <span className='mr_10'>13 August 2023 | 1:30:00 UTC</span> <img src={externalLinkGreenImg} alt='externalLink' /></div>
        </div>
        <div className='dsp'>
          <div> <div className='fnt_wght_500 mb-2'>Starting Balance</div> <div>20.03301761 ETH-CLOUD LP</div></div>
          <div> <div className='fnt_wght_500 mb-2'>Ending Balance </div>  <div>21.03301761 ETH-CLOUD LP</div> </div>
          <div> <div className='fnt_wght_500 mb-2'>Epoch Growth</div>  <div className='primary_header_color'>0.01%</div></div>
          <div> <div className='fnt_wght_500 mb-2'>Epoch APR</div>  <div className='primary_header_color'>0.89%</div></div>
          <div> <div className='fnt_wght_500 mb-2'>Leverage</div>  <div className='primary_header_color'>3.3x</div></div>
        </div>
      </>
    );
  };
    ...
    <>
      ...
      {loading ? <><div className="loader-container">
        ...
        <div className="custom-container notMobileDevice">
          <div className='second_section outer_section_first mb-4 mt-4' style={{ backgroundImage: `url("${homeBackGroundImg}")` }}>
            <div className='dsp_cont'>
              <div className='wdth_40'>
                <div className='holding_header_inner mb-2'>Interest rate swap vaults</div>
                <div className='mb-3 opct_61'>Risk-shared liquidity for DeFi LP Pools</div>
              </div>
          ...
          <div className='second_section outer_section_first mb-4'>
            <div className='dsp_cont'>
              ...
              <span className='opct_61'>Pool TVL</span>
                 <br /> <span className='primary_header_color fnt_wgt_600'>$2.3M</span>
              </div>
              <div>
              <span className='opct_61'>Pool APR</span>
                <br /> <span className='primary_header_color fnt_wgt_600'>15.36 %</span>
              </div>
            </div>
          </div>
                ...
                <div className='dsp mb-4'>
                  <div className='wdth_50 d-flex align-items-center'>
                    <div className='mr_20 redCircle'>
                      <img src={strategyRedImg} alt='chain' className='wdth_35' />
                    </div>
                    <div>
                      <span className='clsHeaderColor'>Strategy is active now</span><img src={externalLinkRedImg} onClick={() => { goToUrl(parentStrategyValContractExplorer) }} alt='externalLinkRedImg' className='extrImgWth crsr_pntr' /><br />
                      <span className='opct_61 mr_15'>Vaults are locked for new deposits & withdrawals. You can supply more assets or redeem your positions at the end of the current epoch.</span>
                    </div>
                  </div>
                  <div>
                    <div className='closeRedBox'>
                     <span className='opct_61'>Epoch closes on</span>   <img src={timerRedImg} alt='chain' /> <span>30 September 2023 | 16:30:00</span>
                    </div>
                  </div>
                </div>
                <div className='dsp'>
                  <div> <div>Strategy LP Balance</div> <div className='fnt_wgt_600'>{startingBalance} ETH-CLOUD LP  </div></div>
                  <div> <div>Overall Gain</div>  <div className='fnt_wgt_600'>2.32 ETH-CLOUD LP</div> </div>
                  <div> <div>Overall Return</div>  <div className='fnt_wgt_600 primary_header_color'>1.02%</div></div>
                  <div> <div>Current APR</div>  <div className='fnt_wgt_600 primary_header_color'>18.23%</div></div>
                  <div> <div>Leverage</div>  <div className='fnt_wgt_600 primary_header_color'>3.2x</div></div>
                </div>
              </div>
            </> :
            <>
              <div className='second_section outer_section_first mb-4'>
                <div className='dsp'>
                  <div className='wdth_50 d-flex align-items-center'>
                    ...
                    <div>
                      <span className='primary_header_color'>Vaults are available now</span><br />
                      <span className='opct_61 mr_15'>You can supply more assets or redeem your positions now. Deposits & withdrawals will be locked once the epoch is running.</span>
                    </div>
                  </div>
                  <div>
                    <div className='firstGreenBox'>
                    <span className='opct_61'>Epoch starts on </span>   <img src={timerImg} alt='chain' /> <span>1 October 2023 | 16:30:00</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {valutList.map((e: any, index: any) => {
                return <div key={index}>
                  ...
                  <div className='first_section outer_section mb-4'>
                    ...
                    <div className='dsp'>
                      <div className='header_font_size'><span><img src={e.assetImg} alt='btc img' className='btc_img_width' /></span>{e.name} <img src={externalLinkGreenImg} onClick={() => { goToUrl(e.contractExplorer) }} alt='externalLink' className='crsr_pntr' /></div>
                      <div>
                        <span className='mr_15 opct_61'>Fixed APR </span><span className='firstGreenBox '>{e.vaultApr}</span>
                      </div>
                    ...
                    <div className='dsp mb-3 mt-3'>
                      <div><span className='opct_61'>Vault TVL</span> <br /> <span className='holding_val'>{Number(e.tvl).toFixed(8)} {e.assetName}</span></div>
                      <div><span className='opct_61'>My Balance</span> <br /> <span>{Number(e?.holding).toFixed(8)} {e.assetName}</span></div>
                    </div>
                    <div className='dsp_around mb-2 mt-4'>
                      {isConnected ?
                        <>
                          ...
                          <div className='wdth_50'>
                            <button disabled={epochRunning} className='btn btn_desgn' onClick={() => { openWithdrawOrDeposit(e, 0) }}><img src={deposit_greenImg} className='wtdrw_depost_btn' alt='deposit' />Depost</button>
                          </div>
                          <div className='wdth_50'>
                            <button disabled={epochRunning} className='btn btn_desgn' onClick={() => { openWithdrawOrDeposit(e, 1) }}><img src={withdraw_greenImg} className='wtdrw_depost_btn' alt='deposit' /> Withdraw</button>
                          </div>
                        </> :
              ...
              {lpValutList.map((e: any, index: any) => {
                return <div key={index}>
                  <div className='first_section outer_section_red'>
                    {/* <div className="small-home-div-3"></div> */}
                    <div className='dsp'>
                      <div className='header_font_size'><span><img src={wETHVaultImg} alt='btc img' className='btc_img_width' /><img src={cloud} alt='btc img' className='btc_img_width ml_20_neg' /></span>{e.name}  <img src={externalLinkRedImg} onClick={() => { goToUrl(e.contractExplorer) }} alt='externalLink' className='crsr_pntr' /></div>
                      <div>
                      <span className='mr_15 opct_61'>Overall APR </span> <span className='firstRedBox'>19.23%</span>
                      </div>
                    ...
                    <div className='dsp mb-3 mt-3'>
                      <div><span className='opct_61'>Vault TVL</span> <br /> <span className='holding_val_red'>{Number(e.tvl).toFixed(8)} {e.assetName}</span></div>
                      <div><span className='opct_61'>My Balance</span> <br /> <span>{Number(e?.holding).toFixed(8)} {e.assetName}</span></div>
                    </div>

                    <div className='redBox mb-3'>
                      <div className='dsp mb-3'>
                        <div className='wdth_50'><span className='opct_61'>Available for borrow</span> <br /> <span className='fnt_wgt_600'>{Number(valutList[0].tvl).toFixed(8)} {valutList[0].assetName} <br/> {Number(valutList[1].tvl).toFixed(8)} {valutList[1].assetName} </span> </div>
                        <div><span className='opct_61'>Available Collateral</span> <br /> <span className='fnt_wgt_600'>{Number(e.tvl).toFixed(8)} {e.assetName}</span> </div>
                      </div>
                      <div className='dsp mb-3'>
                        <div><span className='opct_61'>Projected Leverage</span> <br /> <span className='fnt_wgt_600'>3.2x</span> </div>
                        <div><span className='opct_61'>Max Allowed Leverage</span> <br /> <span className='fnt_wgt_600'>4x</span> </div>
                      </div>
                      <div className='dsp'>
                        <div><span className='opct_61'>Projected APR</span> <br /> <span className='fnt_wgt_600'> 32.6% </span><span className='base_fnt'>Based on Current Pool APR</span></div>
                        <div></div>
                      </div>
                    </div>
                    ...
                    <div className='dsp_around mb-2'>
                      {isConnected ?
                        <>
                          {/* <div><a className='btn btn-riv-secondary view_btn_wdth' href={`#/vault/${e.valutAddress}`}>{buttonVal}</a></div> */}
                          <div className='wdth_50'>
                            <button disabled={epochRunning} className='btn btn_desgn_red' onClick={() => { openWithdrawOrDeposit(e, 0) }}><img src={depositRedImg} className='wtdrw_depost_btn' alt='deposit' />Depost</button>
                          </div>
                          <div className='wdth_50'>
                            <button disabled={epochRunning} className='btn btn_desgn_red' onClick={() => { openWithdrawOrDeposit(e, 1) }}><img src={withdrawRedImg} className='wtdrw_depost_btn' alt='deposit' />Withdraw</button>
                          </div>
                        </> :
                        <><div className='d-flex justify-content-center'><ConnectButton /></div></>}
                    </div>
          ...
          <div className='second_section outer_section_first_red mb-5'>
            <div className='dsclmr_section'>
              ...
              <div>
                <span>Disclaimer:</span> Fixed APR is not risk-free APR. Funds of fixed-return vaults are borrowed for leveraged yield mining.
                All vaults are susceptible to protocol risks & strategy failure risks.
              </div>
              <div>
          ...
          <div className='m10 mb-5'>
            ...
            <AccordionTab header={<CustomAccordianHeader1 icon={epochImg} title="Epoch was live for 2 minutes" />}>
                <div className='dsp wdth_100'>
                  <div> <div>Collateral Deposited</div> <div>5.23 ETH-CLOUD LP  </div></div>
                  <div> <div>Overall  Return</div>  <div>0.34 ETH-CLOUD LP</div> </div>
                  <div> <div>Net Growth</div>  <div>3.6%</div></div>
                  <div> <div>Net APR</div>  <div>43.6%</div></div>
                  <div> <div>Leverage</div>  <div>3.3x</div></div>
                </div>
                <hr />
                <div className='dsp wdth_100'>
                  <div> <div>Borrowed Liquidity</div> <div>80.23 ETH <br/>180,000 CLOUD   </div></div>
                  <div> <div>Overall  Return</div>  <div>0.56 ETH<br/>1,092 CLOUD</div> </div>
                  <div> <div>Net Growth</div>  <div>0.2 %<br/>0.4 %</div></div>
                  <div> <div>Net APR</div>  <div>5 %<br/>10 %</div></div>
                  <div className='wdth4'> <div></div>  <div></div></div>
                </div>
              </AccordionTab>
              <AccordionTab header={<CustomAccordianHeader2 icon={epochImg} title="Epoch was live for 8 hours" />}>
                <div className='dsp wdth_100'>
                  <div> <div>Collateral Deposited</div> <div>7.23 ETH-CLOUD LP  </div></div>
                  <div> <div>Overall  Return</div>  <div>0.34 ETH-CLOUD LP</div> </div>
                  <div> <div>Net Growth</div>  <div>3.6%</div></div>
                  <div> <div>Net APR</div>  <div>43.6%</div></div>
                  <div> <div>Leverage</div>  <div>3.3x</div></div>
                ...
                <hr />
                <div className='dsp wdth_100'>
                  <div> <div>Borrowed Liquidity</div> <div>100.23 ETH <br/>200,000 CLOUD   </div></div>
                  <div> <div>Overall  Return</div>  <div>0.56 ETH<br/>1,092 CLOUD</div> </div>
                  <div> <div>Net Growth</div>  <div>0.2 %<br/>0.4 %</div></div>
                  <div> <div>Net APR</div>  <div>5 %<br/>10 %</div></div>
                  <div className='wdth4'> <div></div>  <div></div></div>
      ...
      <Dialog visible={depositWithDrawVisible} style={{ width: '30vw' }} onHide={() => setDepositWithDrawVisible(false)}>
        <div className='second_section outer_section pos_sticky'>
          <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
            <TabPanel header={<CustomTabHeader icon={deposit_greenImg} title="Deposit" />}>
              <div className='mt-3'>
                ...
                <div className='dsp'>
                  ...
                  <div>{vaultDetail?.walletBalance} {vaultDetail?.assetName}</div>
                </div>
                <div className='dsp'>
                  <div className='buy_cake mt-1 mb-2'>
                    {/* <a target='_blank' href={deatils?.buyToken} className='clr_prpl'>Buy ${deatils?.assetName}</a> */}
                    <a target='_blank' href="#" className='clr_prpl'>Buy ${vaultDetail?.assetName}</a>
                  </div>
                </div>
                {/* <hr />
                <div className='dsp'>
                ...
                <div className='mt-3 text-center'>
                  ...
                  {isConnected ?
                    (chain as any)?.id !== Number("84531") ?
                      <button className='btn btn-riv-primary wdth_100' onClick={() => { networkSwitchHandler(Number("84531")) }} >Switch to Base Goerli (ETH)</button>
                      : isApproved ? <button className='btn btn-riv-primary wdth_100' onClick={deposit} disabled={isTransactionOnGoing}> Deposit</button>
                        : <button className='btn btn-riv-primary wdth_100' onClick={approveIntilize} disabled={isTransactionOnGoing}>Approve</button> :
                    <>
                      <div className='d-flex justify-content-center'><ConnectButton /></div>
            ...
            <TabPanel header={<CustomTabHeader icon={withdraw_greenImg} title="Withdraw" />}>
              <div className='mt-3'>
                <div className='dsp backGrd mb-3'>
                  {/* <div className='fnt_wgt_600'><img src={deatils?.assetImg} className='wdth_50' alt='usdt' /> <br /> {deatils?.assetName}</div> */}
                  <div className='fnt_wgt_600'>{vaultDetail?.assetName}</div>
                ...
                <div className='dsp'>
                  ...
                  {/* <div>{Number(walletBalance).toFixed(2)} {deatils?.assetName}</div> */}
                  <div>{Number(vaultDetail?.holding).toFixed(8)} {vaultDetail?.assetName}</div>
                </div>
                <div className='dsp'>
                  <div className='buy_cake mt-1 mb-2'> <a target='_blank' href="#" className='clr_prpl'>Buy ${vaultDetail?.assetName} </a></div>
                </div>
                {/* <hr />
                <div className='dsp'>
                  <div>Performance Fee</div>
      ...
      <Dialog visible={approvalWaitingVisible} style={{ width: '30vw' }} onHide={() => setapprovalWaitingVisible(false)}>
      ...
      <Dialog visible={approvalSuccessfullVisible} style={{ width: '30vw' }} onHide={() => setapprovalSuccessfullVisible(false)}>
        <div className='text-center mb-5'>
      ...
      <Dialog visible={depositWaitingVisible} style={{ width: '30vw' }} onHide={() => setdepositWaitingVisible(false)}>
        <div className='text-center mb-5'>
          <img src={popupLogoIm} alt='chain' className='gif_wdth' />
      ...
      <Dialog visible={depositSuccessfullVisible} style={{ width: '30vw' }} onHide={() => setdepositSuccessfullVisible(false)}>
        <div className='text-center mb-5'>
          <img src={popupLogoIm} alt='chain' className='gif_success_wdth' />
          <div className='header_font_size'>{transactionType} Successful</div>
          {transactionType === "Withdraw" ? <></> : <div>Great! You are invested now.</div>}
        </div>
      </Dialog>
